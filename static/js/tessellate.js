function createTessellate(den, tessellationLimit=null){
    ...
    tessellationLimit = tessellationLimit == null ? Math.floor(window.innerWidth / 4.5) : tessellationLimit;
    ...
    this.initTessellate = function(density=300, tessellationLimit=null){
        window.tess = createTessellate(density, tessellationLimit);
        tess.getTessellate()
    ...
    let resize_timeout;
    this.destroyTessellate = function(density=300, tessellationLimit=null){
        if(two.scene.children.length){
            two.scene.children.forEach(child => {
                ...
                setTimeout(() => {
                    ...
                    child._renderer.elem.style.opacity = .5;
                }, Math.random() * 300 + 200);
            });
            ...
            if(!resize_timeout){
                resize_timeout = setTimeout(() => {
                    ...
                    document.querySelector('.two-container').remove()
                    initTessellate(density, tessellationLimit)
...
window.onload = () => {
    ...
    window.addEventListener('resize', ev => {
        console.log("window inner width", ev.target.innerWidth, window.prevWidth)
        if (ev.target.innerWidth !== window.prevWidth){
            tess.destroyTessellate();
            window.prevWidth = ev.target.innerWidth
        }
